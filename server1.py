...
def setup_download_folders():
    """Crear carpetas de descarga y configurar permisos"""
    import stat
    folders = [DOWNLOAD_FOLDER, AUDIO_FOLDER, VIDEO_FOLDER]
    for folder in folders:
        os.makedirs(folder, exist_ok=True)
        # Dar permisos de lectura y escritura
        try:
            os.chmod(folder, stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO)
            logging.info(f"✅ Carpeta creada con permisos: {folder}")
        except Exception as e:
            logging.warning(f"⚠️ No se pudieron configurar permisos en {folder}: {e}")
...
def cleanup_old_files():
    """Eliminar archivos antiguos de audio y video"""
    if MAX_FILE_AGE_HOURS is None:
        return
    try:
                ...
                for filename in os.listdir(folder):
                    ...
                    if os.path.isfile(filepath):
                        ...
                        if age_hours > MAX_FILE_AGE_HOURS:
                            os.remove(filepath)
                            logging.info(f"Eliminado archivo antiguo: {filepath}")
    except Exception as e:
...
def download_video():
    ...
    try:
        ...
        if format_type == 'audio':
            output_template = os.path.join(AUDIO_FOLDER, '%(title)s.%(ext)s')
            cmd = [
                'yt-dlp',
...
def serve_file(filepath):
    """Servir archivo descargado desde audio o video"""
    try:
        # Validar que el filepath contenga audio/ o video/
        if filepath.startswith('audio/'):
            full_path = os.path.join(AUDIO_FOLDER, filepath[6:])
        elif filepath.startswith('video/'):
            full_path = os.path.join(VIDEO_FOLDER, filepath[6:])
        else:
